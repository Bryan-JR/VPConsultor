...
buscador.addEventListener('keyup', () => {
    ...
    if (buscador.value.trim() !== "") {
        refresco = setTimeout(async () => {
            ...
            // Cancelar la solicitud anterior si aún está en progreso
            if (cancelToken) {
                cancelToken.cancel("Nueva consulta realizada");
            }

            // Crear un nuevo token para la solicitud actual
            ...
            try {
                const result = await axios.get(`/buscador?search=${encodeURIComponent(buscador.value)}&table=${tab.value}`, {
        ...
        msgTabla(tabBody, ''); // Limpia la tabla si el input está vacío
    }
});
...
function agregarRef(desc, cod, ref, codBarras, pre, dsto, precioDsto) {
    let cant = document.getElementById('nt-'+cod).value;
    if (cant<=0) cant = 0;
    
    if(tab.value=="pulguero") codigo = cod;
    else if(tab.value=="precios"){
...
async function listarProductos(lista) {
    ...
    await lista.forEach(ref => {
        ...
        `;
        if(tab.value == 'pulguero'){
            preciosCol = `
                <td class="px-6 py-4 cursor-help select-none" title="¡¡¡PULGUERO!!!">
                    ---
                </td>
                <td class="px-6 py-4 cursor-help select-none" title="¡¡¡PRECIO PULGUERO!!!">
                    ${pre}
                </td>
            `;
        }
    ...
    document.getElementById('contLista').scrollHeight;
}
